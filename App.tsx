...
function PeriodReport({items,onOpen}) {
  ...
  if(!invalid){
    items.forEach(item=>{
      const ebayHistory=Array.isArray(item._ebaySalesHistory)?item._ebaySalesHistory:[];
      ebayHistory.forEach((sale,index)=>{
        const date=toIsoDate(sale.date);
        if(!date||(startDate&&date<startDate)||(endDate&&date>endDate))return;
        const revenue=parseFloat(sale.grossAmount)||0;
        const ebayNet=parseFloat(sale.netSalesActual)||0;
        const platformFee=Math.max(0,revenue-ebayNet);
        const cogs=parseFloat(sale.cogsAtSale)||0;
        const gross=ebayNet-cogs;
        const tax=gross>0?gross*TAX:0;
        rows.push({key:item.id+"-ebay-"+(sale.dedupeKey||index),item,sale:{...sale,date},quantity:parseFloat(sale.qtySold)||1,revenue,platformFee,cogs,gross,tax,net:gross-tax});
      });
      (item.sales||[]).forEach((sale,index)=>{
      const importedSummary=ebayHistory.length>0&&(/order earnings aggregate/i.test(sale.source||"")||/performance & sales pdf/i.test(sale.source||""));
      if(importedSummary)return;
      const date=toIsoDate(sale.date);
      if(!date||(startDate&&date<startDate)||(endDate&&date>endDate))return;
      const revenue=(parseFloat(sale.price)||0)+(parseFloat(sale.shipCharged)||0);
      const platformFee=revenue*(PLATFORMS[sale.channel||item.channel]?.fee||0);
      const shipping=parseFloat(sale.shipCost)||0;
      const packing=parseFloat(sale.packCost)||0;
      const cogs=parseFloat(sale.cogsAtSale)||parseFloat(sale.costUnitAtSale)||parseFloat(item.costUnit)||0;
      const gross=revenue-platformFee-shipping-packing-cogs;
      const tax=gross>0?gross*TAX:0;
      rows.push({key:item.id+"-manual-"+index,item,sale:{...sale,date},quantity:parseFloat(sale.quantitySold)||1,revenue,platformFee,cogs,gross,tax,net:gross-tax});
      });
    });
  }
  rows.sort((a,b)=>b.sale.date.localeCompare(a.sale.date));
  const sum=field=>rows.reduce((total,row)=>total+row[field],0);
  const label=startDate&&endDate?`${startDate} to ${endDate}`:startDate?`From ${startDate}`:endDate?`Through ${endDate}`:"All dates";
  return <div>
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:16,marginBottom:16}}>
      <div style={{fontSize:16,fontWeight:800,marginBottom:12}}>Profit Report by Period</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))",gap:12}}>
        <FG label="Start date"><Inp val={startDate} set={setStartDate} type="date"/></FG>
        <FG label="End date"><Inp val={endDate} set={setEndDate} type="date"/></FG>
      </div>
      {invalid&&<div style={{color:"#dc2626",fontSize:12,fontWeight:600}}>The start date must be before the end date.</div>}
      {!invalid&&<div style={{color:"#6b7280",fontSize:12}}>{label} · {rows.length} order{rows.length===1?"":"s"} · {sum("quantity")} unit{sum("quantity")===1?"":"s"} sold</div>}
    </div>
    {!invalid&&<>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(150px, 1fr))",gap:10,marginBottom:16}}>
        {[
          {l:"Gross Sales",v:sum("revenue"),c:"#16a34a"},
          {l:"Selling Costs",v:-sum("platformFee"),c:"#dc2626"},
          {l:"Product Cost",v:-sum("cogs"),c:"#dc2626"},
          {l:"Gross Profit",v:sum("gross"),c:"#d97706"},
          {l:"Tax Reserve",v:-sum("tax"),c:"#dc2626"},
          {l:"Net Profit",v:sum("net"),c:sum("net")>=0?"#16a34a":"#dc2626"}
        ].map(card=><div key={card.l} style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,padding:14}}><div style={{fontSize:10,color:"#6b7280",textTransform:"uppercase",marginBottom:4}}>{card.l}</div><div style={{fontSize:20,fontWeight:900,color:card.c}}>{money(card.v)}</div></div>)}
      </div>
      <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,overflow:"hidden"}}>
        <div style={{background:"#f8fafc",borderBottom:"1px solid #e5e7eb",padding:"12px 16px",fontWeight:700,fontSize:14}}>Sales in Period</div>
        {rows.length===0?<div style={{padding:28,textAlign:"center",color:"#9ca3af",fontSize:13}}>No recorded sales in the selected period.</div>:
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#f8fafc"}}>{["Date","ID","Product","Qty","Gross Sale","Selling Costs","Product Cost","Gross Profit","Tax Reserve","Net Profit"].map(h=><th key={h} style={{padding:"9px 10px",textAlign:"left",fontSize:10,color:"#6b7280",textTransform:"uppercase",whiteSpace:"nowrap",borderBottom:"1px solid #e5e7eb"}}>{h}</th>)}</tr></thead>
          <tbody>{rows.map(row=><tr key={row.key} onClick={()=>onOpen(row.item.id)} style={{borderBottom:"1px solid #f3f4f6",cursor:"pointer"}}>
            <td style={{padding:"9px 10px",whiteSpace:"nowrap"}}>{row.sale.date}</td><td style={{padding:"9px 10px",fontFamily:"monospace",fontWeight:700}}>{row.item.id}</td><td style={{padding:"9px 10px",minWidth:210}}>{row.item.name}</td><td style={{padding:"9px 10px",fontWeight:700}}>{row.quantity}</td><td style={{padding:"9px 10px",color:"#16a34a",fontWeight:700}}>{money(row.revenue)}</td><td style={{padding:"9px 10px",color:"#dc2626"}}>{money(-row.platformFee)}</td><td style={{padding:"9px 10px",color:"#dc2626"}}>{money(-row.cogs)}</td><td style={{padding:"9px 10px",fontWeight:700}}>{money(row.gross)}</td><td style={{padding:"9px 10px",color:"#dc2626"}}>{money(-row.tax)}</td><td style={{padding:"9px 10px",fontWeight:800,color:row.net>=0?"#16a34a":"#dc2626"}}>{money(row.net)}</td>
          </tr>)}</tbody>
        </table></div>}
      </div>
    </>}
...
function ERPApp({userEmail,onLogout}) {
  ...
  const TABS=[{id:"dashboard",icon:"📊",label:"Dashboard"},{id:"inventory",icon:"📦",label:"Inventory"},{id:"review",icon:"🧾",label:"Review"},{id:"map",icon:"🗺",label:"Warehouse"},{id:"analytics",icon:"📈",label:"Analytics"},{id:"periodReport",icon:"📅",label:"Period Report"}];
  ...
  return <div style={{fontFamily:"system-ui,-apple-system,sans-serif",minHeight:"100vh",background:"#f4f5f7",color:"#111"}}>
    ...
    <div style={{maxWidth:980,margin:"0 auto",padding:"20px 16px 60px"}}>
      ...
      {tab==="periodReport"&&<PeriodReport items={items} onOpen={id=>setDetailId(id)}/>}
