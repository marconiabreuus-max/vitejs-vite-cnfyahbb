...
async function cloudLoad() { try { const r=await fetch(SB_URL+"/rest/v1/inventory?id=eq.mp_erp_data&select=data",{headers:SB_H}); const rows=await r.json(); return(rows&&rows[0]&&Array.isArray(rows[0].data)&&rows[0].data.length>0)?rows[0].data:null; } catch{return null;} }
async function cloudSave(items) { try { await fetch(SB_URL+"/rest/v1/inventory",{method:"POST",headers:SB_H,body:JSON.stringify([{id:"mp_erp_data",data:items}])}); } catch(e){console.warn("Cloud:",e);} }
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try{
        ...
        if(cloud&&cloud.length>=seed.length){setItems(cloud);localSave(cloud);setCloudOk(true);setLoaded(true);return;}
        const local=localLoad();
        if(local&&local.length>=seed.length){setItems(local);cloudSave(local);setCloudOk(true);setLoaded(true);return;}
  ...
  async function persist(data){setItems(data);localSave(data);cloudSave(data);}
  function toast$(msg,ok=true){setToast({msg,ok});setTimeout(()=>setToast(null),3500);}
  function saveItem(upd){const exists=items.find(i=>i.id===upd.id);persist(exists?items.map(i=>i.id===upd.id?upd:i):[...items,upd]);setEditItem(null);setDetailId(null);toast$("Saved: "+upd.id);}
  function saveSale(upd){persist(items.map(i=>i.id===upd.id?upd:i));setSellId(null);toast$("Sale recorded!");}
  function startNew(){setEditItem({id:genId(items),name:"",sku:"",cat:"Industrial Automation",cond:"uw",qty:1,qtyInStock:1,qtySold:0,supplier:"",invoice:"",lots:"",bought:today(),received:"",listed:"",rack:1,shelf:1,pos:1,notes:"",channel:"ebay",listP:"",listUrl:"",costTotal:"",costUnit:"",status:"purchased",sales:[]});setDetailId(null);}
  ...
  function importFile(e){const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{try{const data=JSON.parse(ev.target.result);if(!Array.isArray(data))throw new Error();persist(data);toast$(data.length+" items loaded!");}catch{toast$("Invalid file",false);}};reader.readAsText(file);e.target.value="";}
  ...
  return <div style={{fontFamily:"system-ui,-apple-system,sans-serif",minHeight:"100vh",background:"#f4f5f7",color:"#111"}}>
    ...
    {detailItem&&!sellId&&!editItem&&<Drawer item={detailItem} onClose={()=>setDetailId(null)} onEdit={()=>{setEditItem(detailItem);setDetailId(null);}} onSell={()=>{setSellId(detailItem.id);setDetailId(null);}} onDup={()=>dupItem(detailItem)}/>}
